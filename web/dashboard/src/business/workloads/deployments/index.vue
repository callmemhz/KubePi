    ...
    return {
      buttons: [
        ...
        },
        {
          ...
          click: (row) => {
            downloadYaml(row.metadata.name + ".yml", getWorkLoadByName(this.clusterName, "deployments", row.metadata.namespace, row.metadata.name))
          },
        },
        {
  ...
  methods: {
        ...
        if (row) {
          ...
          if (this.selects.length > 0) {
            for (const select of this.selects) {
              this.ps.push(deleteWorkLoad(this.clusterName, "deployments", select.metadata.namespace, select.metadata.name))
            }
          }
        }
        if (this.ps.length !== 0) {
    ...
    onScaleSubmit() {
      ...
      for (const row of this.selects) {
        ...
        scaleDeployment(this.clusterName, row.metadata.namespace, row.metadata.name, row)
          .then(() => {
    ...
    onModifyVersionSubmit() {
      ...
      patchDeployment(this.clusterName, this.form.namespace, this.form.name, data)
        .then(() => {
    ...
    onRestart(row) {
      ...
      patchDeployment(this.clusterName, row.metadata.namespace, row.metadata.name, data)
        .then(() => {
          this.dialogModifyVersionVisible = false
          this.loading = true
          this.$message({
    ...
    search(resetPage) {
      ...
      if(!this.isFullTextSearch){
        ...
        listWorkLoads(this.clusterName, "deployments", false)
        .then((res) => {
